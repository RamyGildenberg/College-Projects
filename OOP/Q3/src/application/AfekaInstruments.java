...
public class AfekaInstruments extends Application {
    ...
    public void start(Stage primaryStage) throws Exception {
        ...
        MainView sup = new MainView(allInstruments);
        primaryStage.setScene(sup.getMainScene());
        primaryStage.show();
    }
    ...
    public static File getInstrumentsFileFromUser(){
        ...
        boolean stopLoop = true;
        File file;
        Alert fileUnknown=new Alert(AlertType.ERROR);
        fileUnknown.setTitle("Error");
        fileUnknown.setContentText("Cannot read from file,please try again");
        fileUnknown.setHeaderText("File Error!");
        TextInputDialog  fileInput = new TextInputDialog();
        fileInput.setTitle("Confirmation");
        fileInput.setContentText("Please enter file name");
        fileInput.setHeaderText("Load Instruments From File");
        //Scanner consoleScanner = new Scanner(System.in);

        do {
        	Optional<String> fileName = fileInput.showAndWait();
        	if (!fileName.isPresent()){
        	    System.exit(1);
        	}
        	file=new File(fileName.get());
        	stopLoop=file.exists() && file.canRead();
        	if(!stopLoop)
        	{
        		fileUnknown.showAndWait();
        	}
        }while (!stopLoop);
        return file;
